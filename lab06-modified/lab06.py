...
class Account:
    ...
    def deposit(self, amount):
        ...
        """
        new_transaction = Transaction(len(self.transactions), self.balance, self.balance + amount)
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            new_transaction = Transaction(len(self.transactions), self.balance, self.balance)
            self.transactions.append(new_transaction)
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    # We can either choose the smallest coin or not
    with_smallest = make_change(amount - smallest, rest)
    if with_smallest:
        return [smallest] + with_smallest  # list concatenation
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        # self.coins[coin] = 1 if coin not in self.coins.keys() else self.coins[coin] + 1
        return_change = make_change(coin, self.coins)
        for x in return_change:
            # Do not repeat your self !!!
            # count = self.coins.pop(x) - 1
            # if count:
            #     self.coins[x] = count
            self.coins = remove_one(self.coins, x)  # remove_one returns a new dictionary instead of directly modifying the original dictionary.
